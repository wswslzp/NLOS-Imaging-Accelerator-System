...
case class PixelQuant(cfg: HComplexConfig, quant_bw: Int) extends Component {
  ...
  val out_pix = ( pix_in_rela_inte.payload * pixel_quant_coef ).toUInt.resize(quant_bw)
  io.pix_out << pix_in_rela_inte.translateWith(out_pix).stage()
