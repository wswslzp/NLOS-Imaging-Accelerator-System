...
//  def getMACDatConfig: HComplexConfig = getKernelConfig * getFUinConfig // HCC(16, 12)
//  def getMACDatConfig: HComplexConfig = getKernelConfig * getKernelConfig
...
//    imp_cfg = HComplexConfig(8, 16) >> 3,
  ...
  lazy val uin = Array.tabulate(rsd_cfg.freq_factor){idx=>
    LoadData.loadComplexMatrix(
      real_part_filename = s"src/test/resource/uin/resolutionbar/real/uin_${idx+1}.csv",
      imag_part_filename = s"src/test/resource/uin/resolutionbar/imag/uin_${idx+1}.csv"
    )
  }
