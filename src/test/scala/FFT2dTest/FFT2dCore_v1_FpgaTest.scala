...
object FFT2dCore_v1_FpgaTest extends App{
  ...
  val compiled = if(withWave){
    SimConfig
      ...
      .compile(FFT2dCore_v1(rsd_cfg, rsd_cfg.freq_factor, rsd_cfg.depth_factor))
  } else {
    SimConfig
      .allOptimisation
      .workspacePath("tb")
...
//              huout_d(uout_depth)(r, c) = dut.io.data_to_final.payload(c).toComplex
              huout_d(uout_depth)(r, c) = dut.io.data_to_final.payload.toComplex
              dut.clockDomain.waitSampling()
            }
          }
          uout_depth += 1
